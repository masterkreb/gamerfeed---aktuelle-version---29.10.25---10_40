...
const baseSelectClasses = "bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition w-full capitalize h-[42px]";
...
export const FilterBar: React.FC<FilterBarProps> = (props) => {
  ...
  const isCurrentSearchSaved = searchQuery && savedSearches.includes(searchQuery);

  const handleSaveSearch = () => {
      if (!searchQuery || isCurrentSearchSaved) return;
  ...
  const FilterControls = (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <div ref={searchContainerRef} className="relative flex-grow">
          <input
            ...
            className={`w-full h-[42px] pl-10 pr-10 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition ${searchQuery.length > 0 ? activeFilterClasses : ''}`}
          />
          ...
          {isDropdownOpen && savedSearches.length > 0 && (
            <div className="absolute top-full mt-2 w-full bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 z-10 p-2 space-y-1 animate-fade-in">
                <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 px-2 pb-1 uppercase">Saved Searches</p>
                {savedSearches.map(search => (
                    <div key={search} className="group flex items-center justify-between text-left w-full rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                        <button onMouseDown={() => handleSelectSaved(search)} className="flex items-center gap-2 flex-grow text-left px-3 py-2.5 text-base text-slate-700 dark:text-slate-200 cursor-pointer overflow-hidden">
                           <SearchIcon className="w-5 h-5 text-slate-500 dark:text-slate-400 flex-shrink-0" />
                           <span className="truncate">{search}</span>
                        </button>
                        <button
                            onMouseDown={(e) => handleRemoveSaved(e, search)}
                            className="p-1 mr-2 text-slate-400 hover:text-red-500 dark:hover:text-red-400 rounded-full transition-colors flex-shrink-0 hover:bg-red-500/10"
        ...
        {searchQuery.length > 0 && (
            <button
                ...
                className={`flex-shrink-0 flex items-center justify-center gap-2 px-2 sm:px-4 rounded-lg text-sm font-semibold transition-colors duration-200 border-2 h-[42px] 
                ${ isCurrentSearchSaved
                    ? 'bg-indigo-500 border-indigo-500 text-white'
                    : 'bg-slate-200 dark:bg-slate-700 border-transparent text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600'
                }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <div>
          <label htmlFor="time-filter" className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Time</label>
        ...
        <div>
           <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Language</label>
            <div className={`flex items-center bg-white dark:bg-slate-800 p-1 rounded-lg border h-[42px] transition-all border-slate-300 dark:border-slate-700`}>
                {languageOptions.map(option => (
                    <button
                        key={option.id}
                        onClick={() => handleLanguageFilterChange(option.id as 'all' | 'de' | 'en')}
                        className={`w-full px-2 py-1 rounded-md text-sm font-medium transition-colors h-full flex items-center justify-center gap-1.5 ${
        ...
        <div>
          <label htmlFor="source-filter" className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Source</label>
          <select 
            id="source-filter" 
            value={sourceFilter} 
        ...
        <div className="flex items-end">
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 border-2 h-[42px] ${
              showFavoritesOnly
          ...
          >
            ...
            <span>Favorites</span>
            <span className={`flex items-center justify-center text-xs w-5 h-5 rounded-full ${showFavoritesOnly ? 'bg-yellow-500 text-white' : 'bg-slate-300 dark:bg-slate-600'}`}>
              {favoritesCount}
            </span>
          </button>
        </div>
        ...
        <div className="flex items-end">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 border-2 h-[42px] bg-slate-200 dark:bg-slate-700 border-transparent text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600"
    ...
    <>
      ...
      <div className="lg:hidden">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-md font-semibold transition-all duration-200 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700"
        >
      ...
      <div className="hidden lg:block p-4 bg-white/50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-800">
        {FilterControls}
      </div>
      ...
      />
      <div
        className={`fixed bottom-0 left-0 right-0 z-40 bg-slate-100 dark:bg-slate-900 rounded-t-2xl shadow-2xl transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${
              ...
              <button
                  ...
                  className={`w-full font-bold py-3 px-8 rounded-lg shadow-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                      liveFilteredCount === 0
                          ...
                          : 'bg-indigo-600 hover:bg-indigo-700 text-white hover:shadow-lg transform hover:scale-105 focus:ring-indigo-500 dark:focus:ring-offset-slate-900'
                  }`}
